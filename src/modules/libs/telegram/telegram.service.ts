...
export class TelegramService extends Telegraf {
  ...
  public async onStart(@Ctx() ctx: any) {
    ...
    if (token) {
      ...
      if (!authToken) {
        return await ctx.reply(MESSAGES.invalidToken);
      }
  ...
  public async sendPasswordResetToken(chatId: string, token: string, metadata: SessionMetadata) {
    await this.telegram.sendMessage(chatId, MESSAGES.resetPassword(token, metadata), {
      parse_mode: 'HTML',
    });
  }

  public async sendDeactivateToken(chatId: string, token: string, metadata: SessionMetadata) {
    await this.telegram.sendMessage(chatId, MESSAGES.deactivate(token, metadata), {
      parse_mode: 'HTML',
    });
  }
  ...
  public async sendNewFollowing(chatId: string, follower: User) {
    ...
    await this.telegram.sendMessage(chatId, MESSAGES.newFollowing(follower, followersCount), {
      parse_mode: 'HTML',
    });
  }

  public async sendNewSponsorship(chatId: string, plan: SponsorshipPlan, sponsor: User) {
    await this.telegram.sendMessage(chatId, MESSAGES.newSponsorship(plan, sponsor), {
      parse_mode: 'HTML',
    });
  }
